...
class PatternsTest {
    ...
    @Test fun everyPhaseIsPulsedNotContinuous() {
        for (slot in Patterns.SLOTS) {
            for (p in Patterns.phases(slot)) {
                if (p.kind == Patterns.Kind.FREE) continue      // one deliberate pulse
    ...
    @Test fun pulseRateAndDutyAreHonoured() {
        ...
        assertEquals("8 s at 2.5 Hz = 20 pulses", 20, peaks(Patterns.waveform(Patterns.Kind.HOLD, 8.0, opt)).size)
    }
